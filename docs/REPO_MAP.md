...
- **Screenshots** (`npm run shots`, `tests/e2e/screenshots.spec.ts` + `tests/e2e/playwright.config.ts`): Playwright captures every main route + the three reference agent modals (Dackie / Luda / OPEN CLAW via `/agents?open=<term_id>`) full-page at desktop 1440×900 and mobile 390×844, disconnected wallet only (no wallet fixture exists), into `screenshots/<YYYY-MM-DD>/<desktop|mobile>/<name>.png` (gitignored). Starts `next dev` on :3000 with your `.env.local`, or reuses one already running. A shot whose loading states don't resolve within 90s is still written but its test fails with the reason — a red run means "some PNG shows a loading/error state", not a crash. **One-time local setup: `npx playwright install chromium`** — `@playwright/test` is a devDependency but browsers are never downloaded by `npm ci`, and neither CI nor `npm test` runs Playwright (vitest only matches `src/**`). Fixed chrome (desktop sidebar, mobile bottom nav) paints at its first-viewport position inside full-page PNGs — a capture artifact, not a layout bug.
