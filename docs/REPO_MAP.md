...
### 3. Data Layer
- **No single shared GraphQL client** — the fetch-POST-to-Hasura pattern is reimplemented independently in several files: `api-data.ts` (own `gql<T>()`, server-side, `cache: 'no-store'`), `attestation-reader.ts` and `pnl-engine.ts` (each their own inline `fetch`/`gql`), and `graphql-client.ts` (`graphqlRequest()`, used by client hooks `useTrustScore`/`useAgents`/`useAgent` and `domain-data.ts`, resolves testnet/mainnet URL from `constants.ts` `API.graphql`).
- **Liveness verified (grep, 2026-08-27): `graphql-client` chain consumers — `domain-data.ts`'s `fetchDomainCategoryGraph` IS live, imported by `src/app/lab/poc/page.tsx`; the three hooks (`useTrustScore`, `useAgents`, `useAgent`) are imported by nothing outside their own barrel (`hooks/index.ts`) — dead code. A pending cleanup stash could safely drop the three hooks but must NOT touch `domain-data.ts`.**
...
- **Shared libs**: `src/lib/` (business logic, scoring, data fetching — no React), `src/components/` (UI), `src/hooks/` (client-side data hooks wrapping `graphql-client.ts`).
