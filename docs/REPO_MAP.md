...
1. **Capped fetches report their own truncation.** Any query with a `limit` returns `{ rows, total, truncated }`, not a bare capped array — `total` comes from a separate aggregate count query on the SAME filter, never from `rows.length` (the only way `total` stays true once the fetch itself is capped — counting what you fetched just re-reports the cap). The UI surfaces `"showing first N of {total}"` whenever `truncated` is true. Never silently drop (thesis §6). Reference implementation: `lib/cohort-reader.ts`'s `fetchCohortAgents()` — `COHORT_FETCH_LIMIT = 500` on the identity query, a `triples_aggregate` query on the identical `where` for `total`, surfaced in `/agents`'s header line. (3e3e2a9, d8e74d2.) **`total` must be in the same unit as the rows**: the cohort count is `count(columns: [subject_id], distinct: true)` (agents, not same-as triples — an agent with two identity links is one row). A fetch that returned fewer rows than its cap is complete, so its row count IS the total and no count is needed; only at the cap does the aggregate decide, and if it failed there `total`/`truncated` are `null` (unknown) — never a substituted `rows.length`. Same pattern: `lib/agent-list.ts` `fetchAgentListCorpus()` (rows + `atoms_aggregate` in one request, `listTruncation()`), `api-data.ts` `loadAgentCorpus()`. The `/agents` header prints corpus totals only (never search-dependent); the results line prints the filtered count.
